...
from Preprocessing.moview_rationale_pre1 import RationaleDataset
from modeling.movie_rationale import (
...
def main():
    ...
    classifier = AlbertForSequenceClassification.from_pretrained(config.pretrained_classifier_path)
    ...
    if training_args.do_train:
        ...
        for epoch in range(num_epochs):
            ...
            train_result_file_name = f"train_results_suf_{model.config.suf_weight}_cpt_{model.config.cpt_weight}_lf{model.config.lf_weight}.xlsx"  # 动态命名文件
            eval_result_file_name = f"eval_results_suf_{model.config.suf_weight}_cpt_{model.config.cpt_weight}_lf{model.config.lf_weight}.xlsx"  # 动态命名文件
