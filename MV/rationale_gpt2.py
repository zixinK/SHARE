...
from Preprocessing.moview_rationale_pre_gpt2 import RationaleDataset
from modeling.movie_rationale_gpt2 import (
    GPT2ForTokenRationale,
    compute_metrics_fn,
)
...
class ModelArguments:
    ...
    mlp_weight: Optional[float] = field(
        default=0.05, metadata={"help": "Lambda for mlp regression."}
    )
...
class CustomGPT2ForTokenRationale(GPT2ForTokenRationale):
    def get_input_embeddings(self):
        return self.transformer.wte  # 这里返回 GPT-2 的输入嵌入层（即 `wte`）
...
def main():
    ...
    # tokenizer.mask_token_id = tokenizer(tokenizer.mask_token)['input_ids']
    ...
    model = GPT2ForTokenRationale.from_pretrained(
    ...
    classifier = GPT2ForSequenceClassification.from_pretrained(config.pretrained_classifier_path)
    ...
    train_dataset = DataLoader((
        RationaleDataset(data_args, tokenizer=tokenizer, classifier=classifier, neighbor=config.num_neighbors,
                         mode="train", cache_dir=data_args.data_cache_dir)
        if training_args.do_train
        else None
    ), batch_size=training_args.per_device_train_batch_size)
    eval_dataset = DataLoader((
        RationaleDataset(data_args, tokenizer=tokenizer, classifier=classifier, neighbor=config.num_neighbors,
                         mode="dev", cache_dir=data_args.data_cache_dir)
        if training_args.do_eval
        else None
    ), batch_size=training_args.per_device_eval_batch_size)
    test_dataset = DataLoader((
        RationaleDataset(data_args, tokenizer=tokenizer, classifier=classifier, neighbor=config.num_neighbors,
                         mode="test", cache_dir=data_args.data_cache_dir)
    ...
    if training_args.do_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_dataset:
                ...
                # model.config.cph_weight = ((np.mean(outputs[-5])+1e-12)/(old_length+1e-12))*0.3
                model.config.cph_weight = (np.mean(outputs[-7]) + 1e-12)
                # old_length = np.mean(outputs[-5])+1e-12
        ...
        train_result_file_name = f"train_results_gpt2_suf_{model.config.suf_weight}_cpt_{model.config.cpt_weight}_lf{model.config.lf_weight}.xlsx"  # 动态命名文件
        eval_result_file_name = f"eval_results_gpt2_suf_{model.config.suf_weight}_cpt_{model.config.cpt_weight}_lf{model.config.lf_weight}.xlsx"  # 动态命名文件
