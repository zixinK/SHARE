...
def compute_metrics_fn(eval_prediction, tokenizer=None, return_text=False, nei_index=2):
    ...
    for i in range(len(eval_prediction[1])):
        ...
        is_masked = np.greater(mask, 0.5).astype(np.int).tolist()
        ...
        for j in range(len(mask_labels)):
            ...
            ground_r.append(mask_labels[j].astype(np.int).tolist())
            f1.append(metrics.f1_score(is_masked[j],mask_labels[j].astype(np.int).tolist(), average="micro"))
            recall.append(metrics.recall_score(is_masked[j],mask_labels[j].astype(np.int).tolist(), average="micro"))
            precision.append(metrics.precision_score(is_masked[j],mask_labels[j].astype(np.int).tolist(), average="micro"))

            e_d= Levenshtein.distance(is_masked[j],mask_labels[j].astype(np.int).tolist())
            ed.append(e_d)
    ...
    np.save('mv_test_gpt2_gtr_lf_pred.npy', lf_pred)
    np.save('mv_test_gpt2_gtr_nolf_pred.npy', nolf_pred)    
...
class GPT2ForTokenRationale(GPT2PreTrainedModel):

    def __init__(self, config):
        super(GPT2ForTokenRationale, self).__init__(config)
        self.config = config
        self.num_labels = config.num_labels
        self.nei_index = config.nei_index
        self.mask_token_id = config.mask_token_id
        self.GPT2 = GPT2Model(config).to(device)
        for pp in self.GPT2.parameters():
            pp.requires_grad = True
        self.dropout = nn.Dropout(config.resid_pdrop).to(device)
        self.masker = nn.Linear(config.n_embd, 2).to(device)
        ...
        if not self.config.use_pretrained_classifier:
            ...
            self.init_weights()
            self.classifier = GPT2ForSequenceClassification.from_pretrained(config.pretrained_classifier_path).to(device)
        ...
        **kwargs,
        ...
        if self.training:
            ...
            for i in range(len(evidence_mask)):
                ...
                idx = torch.cat((torch.tensor(range(int(512-int(attention_mask.sum(dim=1)[i])))),idx,torch.tensor(range(512-int(attention_mask.sum(dim=1)[i])+int(evidence_mask.sum(dim=1)[i]),512)))).to(device).long()
                new_weights = torch.cat((new_weights,torch.gather(rationale_weights[i].clone(),0,idx).unsqueeze(0))).to(device)
