...
# @profile
def construct_positions_connectedshapley(d,d_a, d_s, k, max_order=None):
    ...
    # print(values)
    values = [
        np.pad(np.array(value[0].tolist()), (max(d_s-d_a,0), d_a-d), 'constant', constant_values=(0, 1)).reshape(
...
def explain_shapley(model, d, k, key_to_idx, po_inputs, inputs, coefficients, unique_inverse, attention_mask,
    ...
    # start1=time.time()
    epo = math.ceil(len(po_inputs) / 200)
    f_vals = []
    # print(po_inputs.shape)
    with torch.no_grad():
        for i in range(epo):
            if i != epo - 1:
                ddd = po_inputs[i * 200:(i + 1) * 200].to(device)
                # print(ddd.shape)
                f_result = model(input_ids=ddd, attention_mask=attention_mask[i * 200:(i + 1) * 200],
                                 token_type_ids=token_type_ids[i *200:(i + 1) * 200]).logits
                f_vals.extend(torch.nn.functional.softmax(f_result, dim=-1).tolist())
            else:
                ddd = po_inputs[i *200:].to(device)
                f_result = model(input_ids=ddd, attention_mask=attention_mask[i * 200:],
                                 token_type_ids=token_type_ids[i * 200:]).logits
                f_vals.extend(torch.nn.functional.softmax(f_result, dim=-1).tolist())
