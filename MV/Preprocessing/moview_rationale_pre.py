...
from Preprocessing.connected_shapley import explain_shapley, construct_positions_connectedshapley
...
def convert_fn(examples, tokenizer, label_list,classifier,neighbor, max_length=None, verbose=True):
    ...
    for example in tqdm.tqdm(examples, "converting to features"):
        mask_labels = []
        all_evidence_tokens = []
        # example.text_a = pre_document(example.text_a)
        ...
        if mask_labels:
            ...
            mask_labels = [0] + mask_labels + [0] * (max_length - len(mask_labels) - 1)
        # else:
        #     # is_unsupervised = 1
        ...
        atten_m = (encoding['attention_mask'].repeat(len(positions),1)).to(device)
        atten_m = (atten_m*mask_weights).squeeze().to(device)
        ...
        score = [0] +score+ [0] * (max_length - int(torch.Tensor(evidence_mask).sum(dim=0))-1)
