...
from Preprocessing.connected_shapley_t5 import explain_shapley, construct_positions_connectedshapley,explain_shapley_no
from Preprocessing.t5_encode import encode_for_training
...
class RationaleProcessor(DataProcessor):
    ...
    def _read_jsonlines(input_file):
        ...
        # l_len = len(lines)
        # random.seed(1234)
        # line_r = [random.randint(0,l_len-1) for _ in range(int(l_len*0.005))]
...
def convert_fn(examples, tokenizer, label_list,classifier,neighbor, max_length=None, verbose=True):
    ...
    for example in tqdm.tqdm(examples, "converting to features"):
        ...
        evidence_mask = [1.0] * min(max_length - num_special, len(all_evidence_tokens))
        evidence_mask = evidence_mask + [0.0] * (max_length - len(evidence_mask))

        if mask_labels:
            # is_unsupervised = 0
            mask_labels = mask_labels[:max_length - num_special]
            mask_labels = mask_labels + [0] * (max_length - len(mask_labels))
        # else:
        #     # is_unsupervised = 1
        ...
        if example['label']:
            labels= [int(label_map[example['label']])]
            label = example['label']
            # print(label)
        else:
            print(example)
        ...
        positions_dict, key_to_idx, positions, coefficients, unique_inverse = construct_positions_connectedshapley(int(torch.Tensor(evidence_mask).sum(dim=0)), int(len(encoding['input_ids'][0])),neighbor)
        
        pos_encoding = tokenizer(
        'POS', padding="longest", max_length=7, truncation=True
        )
        label_id_pos = pos_encoding.input_ids
        label_id_pos = torch.tensor(label_id_pos)
        label_id_pos[label_id_pos == tokenizer.pad_token_id] = -100

        
        neg_encoding = tokenizer(
        'NEG', padding="longest", max_length=7, truncation=True
        )
        label_id_neg = neg_encoding.input_ids
        label_id_neg = torch.tensor(label_id_neg).to(device)
        label_id_neg[label_id_neg == tokenizer.pad_token_id] = -100
        
        label_encoding = tokenizer(
        label, padding="longest", max_length=7, truncation=True
        )
        label_id = label_encoding.input_ids
        label_id = torch.tensor(label_id).to(device)
        label_id[label_id == tokenizer.pad_token_id] = -100


        
        po_evd = [example['text_a'] for _ in range(len(positions))]
        po_label = [label for _ in range(len(positions))]
        train_dataloader = encode_for_training(po_evd,po_label,512,'t5-small',positions)
        
        train_dataloader1 = encode_for_training(claims = [example['text_a']],labels = [label],batch_size=1,model = 't5-small')

        # print(len(positions))
        # print(int(torch.Tensor(evidence_mask).sum(dim=0)))
        # print('???????????????????????????????????????')
        score = explain_shapley(classifier,int(torch.Tensor(evidence_mask).sum(dim=0)), neighbor, key_to_idx,
                                coefficients, unique_inverse,train_dataloader,train_dataloader1,label,tokenizer,label_id_pos,label_id_neg)

        # score = sigmoid_function(score)
        # score = torch.nn.functional.softmax(torch.tensor(score).to(device), dim=-1).tolist()
        score = score+ [0] * (max_length - int(torch.Tensor(evidence_mask).sum(dim=0)))

        score1 = explain_shapley_no(classifier,int(torch.Tensor(evidence_mask).sum(dim=0)), neighbor, key_to_idx,
                                coefficients, unique_inverse,train_dataloader,train_dataloader1,label,tokenizer,label_id_pos,label_id_neg)

        score1 = score1 + [0] * (max_length - int(torch.Tensor(evidence_mask).sum(dim=0)) )
        ...
        feature = {'input_ids':encoding['input_ids'].squeeze(),
            ...
            'labels':torch.Tensor(labels),
                   'labels_encode':label_id,
            'mask_labels':torch.Tensor(mask_labels),
                  'pos_encode':label_id_pos,
                  'neg_encode':label_id_neg}
