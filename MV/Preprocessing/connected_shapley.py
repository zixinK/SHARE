...
def construct_positions_connectedshapley(d, d_s, k, max_order=None):
    ...
    values = [np.pad(np.array(value[0]),(0,d_s-d),'constant',constant_values=(0,1)).reshape(-1, d_s) for value in values]
...
# @profile
def explain_shapley(model,d, k, key_to_idx,po_inputs, inputs, coefficients, unique_inverse,attention_mask,token_type_ids,attention_m,token_type_i,label):
    """
    ...
    # start1=time.time()
    epo = math.ceil(len(po_inputs)/128)
    f_vals = []
    # print(po_inputs.shape)
    with torch.no_grad():
        for i in range(epo):
            if i != epo-1:
                ddd = po_inputs[i*128:(i+1)*128].to(device)
                # print(ddd.shape)
                f_result = model(input_ids = ddd,attention_mask = attention_mask[i*128:(i+1)*128],token_type_ids = token_type_ids[i*128:(i+1)*128]).logits
                f_vals.extend(torch.nn.functional.softmax(f_result, dim=-1).tolist())
            else:
                ddd = po_inputs[i*128:].to(device)
                f_result = model(input_ids = ddd,attention_mask = attention_mask[i*128:],token_type_ids = token_type_ids[i*128:]).logits
                f_vals.extend(torch.nn.functional.softmax(f_result, dim=-1).tolist())
    ...
    p_result = model(input_ids = inputs.to(device),attention_mask = attention_m.to(device),token_type_ids = token_type_i.to(device)).logits
